    ...
    let rec loop state = actor {
        ...
        | VoteReq update ->
            ...
            let state' =
                if vote then
                    ...
                    { state with
                        commitState = ParticipantInitCommit (update, upSet) }
                else
                    startObserverHeartbeat {
                        state with
        ...
        | PreCommit ->
            ...
            | ParticipantInitCommit (update, upSet) ->
                let state' = {
                    state with
                        commitState = ParticipantCommitable (update, upSet)}
